...
namespace urlbliss {
    class Program {
        ...
        private static WebProxy initializeProxyConfigs() {
            try {
                ...
                if (proxy != null) { // set grabbed proxy settings to myproxy
                    ...
                    throw new ArgumentException("\n │\t├─ !No proxy detected.\n\tSet proxy to \"EDIT_YOUR_PROXY_ADRESS_IN_CSFILE\"");
                    Uri newUri = new Uri("YOUR_PROXY_ADRESS");
        ...
        /// <summary> Calls related funtions depends on choice at the Main Menu </summary>
        static void makeChoice() {
            ...
            switch (Console.ReadLine()) {
                ...
                case "6":
                    ...
                    break;
                default:
                    Console.WriteFormatted("\t ╟ Wrong Input!!!",red);
                    break;
            }
            Console.Write("\n\n\t│ Press Enter to try Again. ");
        }
        ...
        static void checkFileContent() {
            ...
            using (StreamReader file = new StreamReader(urlblissFileName)) {
                if (file.EndOfStream == true) {
                    Console.WriteFormatted("\n │ ! File is empty as my dreams.", red);
                    return;
                }
                if (file.ReadLine() == null) {
                    Console.WriteFormatted("\n │ ! Failure when reading file.", red);
